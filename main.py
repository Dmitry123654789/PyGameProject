        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.WINDOWRESIZED:
                    if screen.get_size()[0] < 800:
                        pygame.display.set_mode((800, screen.get_size()[1]), pygame.RESIZABLE)
                    if screen.get_size()[1] < 600:
                        pygame.display.set_mode((screen.get_size()[0], 600), pygame.RESIZABLE)
                    if (self.width, self.height) != screen.get_size():
                        ofset = (self.width / 2 - screen.get_width() / 2), (self.height / 2 - screen.get_height() / 2)
                        self.field.shift_sprites(*ofset)
                        self.player.shift_player(ofset[0] * -1, ofset[1] * -1)
                        # print((self.width / 2 - screen.get_width() / 2), (self.height / 2 - screen.get_height() / 2), self.width, self.height, screen.get_size())
            ...
            self.update_sprites()
            self.draw_sprites()
            print(self.player.hitbox.center, screen.get_size())
